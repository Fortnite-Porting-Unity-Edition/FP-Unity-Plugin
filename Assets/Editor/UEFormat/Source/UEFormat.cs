...
public class UEFormat
{
    ...
    private static bool ProcessAsset(string path)
    {
        ...
        if (header.IsCompressed)
        {
            ...
            switch (compressionType)
            {
                case "GZIP":
                    ...
                    var srcStream = new MemoryStream(ar.ReadToEnd());
                    var gzipStream = new GZipStream(outStream, CompressionMode.Decompress);
                    srcStream.CopyTo(gzipStream);
                    readArchive = new FArchiveReader(outStream.ToArray());
                    break;
                case "ZSTD":
        ...
        {
            case "UEMODEL":
                UEFModelReader.ImportUEModelData(readArchive, header, importOptions);
                break;
            case "UEANIM":
                break;
            default:
